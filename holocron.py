#!/usr/bin/python3
# -*- coding: utf-8 -*-
# Made by Alexander Bigulov 05.09.2021. Github: https://github.com/bialger/Holocron
...
try:  # We import some necessary libraries, packages and classes ...
    import hashlib
    import random
    import datetime
    import os
    ...
    from PyQt5.QtWidgets import (QWidget, QLineEdit, QTextEdit, QGridLayout, QDesktopWidget, QMainWindow, QLabel,
                                 QToolTip, QComboBox, QPushButton, QMessageBox, QApplication, QFileDialog)
...
class Holocron(QWidget):  # Declare the main class. May the Force be with you! :)
    def color(self, cur_object,
              color="green"):  # We declare a method that changes the font color. It will come in handy many more times.
    ...
    def initUI(self):  # Declare the main method
        ...
        self.setToolTip(
            'Holocron 1.0 by bialger')  # Set up a tooltip for the entire window, later configure for individual parts
        ...
        self.lne1.setToolTip(
            'Выберите сложный пароль, из минимум 8 символов, используя прописные и строчные латинские буквы, цифры и спецсимволы')
        ...
        self.btn2.setToolTip('Выходной файл по умолчанию, вида [имя_исходного_файла].holocron')
        ...
        self.btn.clicked.connect(
            self.crypt)  # We connect with processing methods all objects with which the user interacts
        ...
        self.setWindowTitle('Holocron 1.0 Personal Jesus')  # Customize the window title
    ...
    def chooseInfile(self):
        ...
        if file_dialog[0] != "":
            ...
            if self.de == "en":
                self.lne3.setText(file_dialog[0] + ".holocron")
                self.lne23.setText(file_dialog[0] + ".key")
                self.outfile = file_dialog[0] + ".holocron"
                self.file2 = file_dialog[0] + ".key"
                msg = 'Итоговый файл: '
                if self.lan == 'en':
                    msg = 'End file: '
                self.lbl33.setText(msg + basename(file_dialog[0] + ".holocron"))
                msg = 'Файл-ключ: '
                if self.lan == 'en':
                    msg = 'Key file: '
    ...
    def ch_lan(self, text):
        ...
        if text == 'Русский':
            ...
            self.btn2.setToolTip('The default output file, of the form [source_name].holocron')
            if self.de == 'en':
                self.btn3.setToolTip('Default key file of the form [source_name].key')
            else:
                self.chb3.setToolTip('Opens the key file selection window')
            ...
            self.lbl213.setText(
                'Enter the full path to the key file. Leave blank to use Crypt0                         ')
